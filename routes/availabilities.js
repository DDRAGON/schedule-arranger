...
router.post(
  ...
  async (req, res, next) => {
    await body('availability').isInt({ min:0, max: 2 }).withMessage('出欠は0以上2以下の数値を指定してください。').run(req);
    await param('scheduleId').isUUID('4').withMessage('有効なスケジュールIDを指定してください。').run(req);
    await param('candidateId').isInt().withMessage('有効な候補日IDを指定してください。').run(req);
